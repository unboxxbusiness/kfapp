...
async function testLead() {
  ...
  // Test 3: Write to advertiser_leads
  const res3 = await fetch(`https://firestore.googleapis.com/v1/projects/${projectId}/databases/(default)/documents/advertiser_leads`, {
    method: 'POST',
    headers: {
      Authorization: `Bearer ${token}`,
      'Content-Type': 'application/json',
    ...
    body: JSON.stringify({
      fields: {
        name: { stringValue: 'Dr. Test Chancellor' },
        designation: { stringValue: 'Chancellor' },
        email: { stringValue: 'chancellor@testuniv.edu.in' },
        phone: { stringValue: '+91 97777 66666' },
        university: { stringValue: 'Test Apex University' },
        stream: { stringValue: 'Engineering & Technology' },
        budget: { stringValue: '₹30L – ₹60L' },
        status: { stringValue: 'new' },
        source: { stringValue: 'advertise_page' },
        serverTimestamp: { timestampValue: new Date().toISOString() }
      }
    })
  });
  const data3 = await res3.json();
  console.log('Test 3 (advertiser_leads) result status:', res3.status, 'ID:', data3.name);
}
