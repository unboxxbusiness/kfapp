...
export function SmashingPagination({
  ...
  const getPageUrl = (page: number) => {
    // 1. Clean path-based pagination for Home (basePath is '/' or empty)
    if (!basePath || basePath === '/') {
      return page <= 1 ? '/' : `/page/${page}`;
    }

    // 2. Clean path-based pagination for Categories (e.g. /category/mba)
    const cleanBase = basePath.replace(/\/page\/\d+$/, '').replace(/\/$/, '');
