...
interface CategoryFeedProps {
  ...
  activeCategories: { label: string; value: string; slug?: string }[];
  currentPage: number;
  paginatedArticles: ArticleRow[];
  totalArticles: number;
  totalPages: number;
  articlesPerPage: number;
  collectionJsonLd: any;
}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10 space-y-6">
          ...
          <div className="max-w-4xl space-y-4">
            ...
            <h1 className="font-serif text-3xl sm:text-5xl lg:text-6xl font-black text-[#14213D] tracking-tight leading-[1.12]">
              {meta.headline}
              {!isFirstPage && (
                <span className="block text-2xl sm:text-3xl text-[#14213d]/70 font-sans font-bold mt-2">
          ...
          <div className="pt-4 border-t-2 border-[#14213d]/15">
            <div className="flex items-center justify-between gap-2 mb-2.5">
              ...
              <span className="text-[11px] font-bold text-[#14213D]/50 hidden sm:inline">
                {totalArticles} Guides in {meta.label} &bull; 94 Clusters Total
              </span>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-2.5">
              ...
              <Link
                href="/"
                className="px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider transition-all border-2 border-[#000000] bg-white text-[#14213D] shadow-[2px_2px_0_0_#14213D] hover:bg-[#FCA311] hover:text-[#000000] hover:shadow-[3px_3px_0_0_#000000]"
              >
                All Guides
              </Link>

              {/* Current Active Category Pill */}
              <span className="px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider border-2 border-[#000000] bg-[#FCA311] text-[#000000] shadow-[3px_3px_0_0_#000000] translate-y-[-1px] inline-flex items-center gap-1.5">
                <CheckCircle2 className="w-3.5 h-3.5" />
                <span>{meta.label}</span>
                <span className="px-1.5 py-0.2 rounded-full text-[10px] bg-black text-[#FCA311] font-bold">
                  {totalArticles}
                </span>
              </span>

              {/* Top Sibling Categories (exclude current and 'All Guides') */}
              {activeCategories
                .filter(
                  (cat) =>
                    cat.slug !== slug &&
                    cat.value &&
                    cat.slug &&
                    cat.label.toLowerCase() !== 'all guides'
                )
                .slice(0, 6)
                .map((cat) => (
                  <Link
        ...
        <section>
          ...
          {gridArticles.length > 0 ? (
            ...
            <div className="text-center py-16 px-4 rounded-2xl border-2 border-dashed border-[#14213d]/30 bg-white">
              ...
              >
                <span>Return to Category Page 1</span>
                <ArrowRight className="w-3.5 h-3.5 text-[#FCA311]" />
              </Link>
            </div>
          )}

          {/* Dedicated Category Clean Path Pagination */}
          <SmashingPagination
